//var apiBase = "https://jsonplaceholder.org/";
//console.log(apiBase);
...
async function getResponse() {
  ...
  //console.log(typeof uniqueMonthYears);
  ...
  // это сделано, чтобы не было Invalid Date
...
fetch(apiBase + "posts")
  ...
  .then(function (json) {
    const dates = getDatesOfPosts(json);
    const sortedDates = sortDates(dates);
    const monthYears = getMonthAndYearOfDates(sortedDates);
    const uniqueMonthYears = getArrayUniqueValues(monthYears);
    putArrayIntoHtml(uniqueMonthYears);
  })
  .catch(function (err) {
    console.error(err);
  });
...
function getArrayUniqueValues(arr) {
  return (uniqueArr = Array.from(new Set(arr)));
}
